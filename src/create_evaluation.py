...
if __name__ == '__main__':
    ...
    hold_images_path = 'evaluation/data/hold_out_images.npz'
    test_images = np.load(hold_images_path, allow_pickle=True)['input_images']

    # Load your model here. If you use different parameters for the initialization you can change
    ...
    for i, numpy_image in enumerate(test_images, 0):
        ...
        with torch.no_grad():
            ...
            boxes, scores, labels = boxHead.postprocess_detections(
                class_logits, box_pred, proposals,
                conf_thresh=0.8, keep_num_preNMS=200, keep_num_postNMS=3)
