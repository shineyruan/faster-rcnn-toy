...
if __name__ == "__main__":
  ...
  with torch.no_grad():
    ...
    for iter, batch in enumerate_tqdm:
      ...
      proposal_labels, regressor_target = box_head.create_ground_truth(
          proposals, labels, bbox)
      class_logits, box_preds = box_head.forward(
          feature_vectors, training=False)
      nms_boxes, nms_scores, nms_labels = box_head.postprocess_detections(
          class_logits, box_preds, proposals, keep_num_postNMS=2)
      top_boxes, _, top_labels = box_head.get_top_K(class_logits, box_preds,
                                                    proposals)
      ...
      for i in range(images.shape[0]):
        ...
        labels_fg = proposal_labels[start_id:start_id +
                                    proposals[i].shape[0]][~bg_ids] - 1
        out_img = visual_bbox_mask(
            images[i].cpu(),
            proposals_fg.cpu(),
            labels_fg.cpu(),
            gt_bbox=bbox[i])
        image_path = os.path.join(
            images_path,
            'visual_output_' + str(iter) + '_' + str(i) + 'proposal_gt.png')
        ...
        image_path = os.path.join(
            images_path,
            'visual_output_' + str(iter) + '_' + str(i) + 'top_K.png')
        cv2.imwrite(image_path, out_img)
        cv2.imshow("visualize output", out_img)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
        ...
        image_path = os.path.join(
            images_path,
            'visual_output_' + str(iter) + '_' + str(i) + 'after_nms.png')
        cv2.imwrite(image_path, out_img)
        cv2.imshow("visualize output", out_img)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
