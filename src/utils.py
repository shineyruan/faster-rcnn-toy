...
def visual_bbox_mask(image: torch.Tensor,
                     bboxes: torch.Tensor,
                     labels: torch.Tensor,
  ...
  out_img = (out_img * np.array([0.229, 0.224, 0.225]) +
  ...
  for i in range(bboxes.shape[0]):
    ...
    out_img = cv2.rectangle(out_img, (int(x1), int(y1)), (int(x2), int(y2)),
                            color, 3)

    if scores is not None:
      out_img = cv2.putText(out_img, "Score: {}".format(scores[i]),
                            (int(x1), int(y1)), cv2.FONT_HERSHEY_SIMPLEX, 1.0,
                            (20, 200, 200), 2)
  ...
  if gt_bbox is not None:
    for i in range(gt_bbox.shape[0]):
      if bbox_format == 'corner':
        ...
        x_gt, y_gt, w_gt, h_gt = gt_bbox[i][0], gt_bbox[i][1], gt_bbox[i][
            2], gt_bbox[i][3]
        x1_gt = x_gt - w_gt / 2
        y1_gt = y_gt - h_gt / 2
        x2_gt = x_gt + w_gt / 2
...
def matrix_IOU_center(boxA, boxB, device='cpu'):
  ...
  # box: (grid_x, grid_y, 4)
  ##################################
  inter_x1 = torch.max(boxA[..., 0] - boxA[..., 2] / 2,
                       boxB[..., 0] - boxB[..., 2] / 2)
  inter_y1 = torch.max(boxA[..., 1] - boxA[..., 3] / 2,
                       boxB[..., 1] - boxB[..., 3] / 2)
  inter_x2 = torch.min(boxA[..., 0] + boxA[..., 2] / 2,
                       boxB[..., 0] + boxB[..., 2] / 2)
  inter_y2 = torch.min(boxA[..., 1] + boxA[..., 3] / 2,
                       boxB[..., 1] + boxB[..., 3] / 2)
  ...
  iou = inter / (
      boxA[..., 2] * boxA[..., 3] + boxB[..., 2] * boxB[..., 3] - inter + 1)
  return iou
...
def corner_to_center(bbox):
  ...
  return torch.tensor([(bbox[0] + bbox[2]) / 2, (bbox[1] + bbox[3]) / 2,
...
def center_to_corner(bbox):
  ...
  return torch.tensor([
      bbox[0] - bbox[2] / 2, bbox[1] - bbox[3] / 2, bbox[0] + bbox[2] / 2,
      bbox[1] + bbox[3] / 2
  ])
