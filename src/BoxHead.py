...
from utils import (matrix_IOU_corner, corner_to_center, center_to_corner,
                   corners_to_centers, centers_to_corners, IOU)
...
class BoxHead(torch.nn.Module):
  ...
  def __init__(self, device='cuda', Classes=3, P=7):
    ...
    self.intermediate = nn.Sequential(
        nn.Linear(in_features=256 * P * P, out_features=1024), nn.ReLU(),
  ...
  def create_ground_truth(self, proposals: list, gt_labels: list,
                          gt_bboxes: list):
    """
    ...
    for batch_id in range(len(gt_labels)):
      ...
      proposal_mat = proposals[batch_id].expand(num_gtbboxes, num_proposals,
                                                4).to(self.device)
      ...
      proposal_labels = torch.zeros(
          num_proposals, 1, dtype=torch.long).to(self.device)
      regressor = torch.zeros(num_proposals, 4).to(self.device)
      for i in range(num_proposals):
        if not background[i]:
          proposal_labels[i, 0] = gt_labels[batch_id][matched_gtbbox.indices[i]]

        gt_bbox_center = corner_to_center(
            gt_bboxes[batch_id][matched_gtbbox.indices[i]])
        proposal_center = corner_to_center(proposals[batch_id][i])
        regressor[i, 0] = (gt_bbox_center[0] -
                           proposal_center[0]) / proposal_center[2]
        regressor[i, 1] = (gt_bbox_center[1] -
                           proposal_center[1]) / proposal_center[3]
  ...
  def MultiScaleRoiAlign(
      self,
      fpn_feat_list: list,
      proposals: list,
      P: int = 7,
      img_size: tuple = (800, 1088)) -> torch.Tensor:
    """
    ...
    roi_fn = torchvision.ops.MultiScaleRoIAlign(
        fpn_names, (P, P), sampling_ratio=-1)
    feature_vectors = roi_fn(fpn_dict, proposals, [img_size])

    return torch.reshape(feature_vectors,
                         (feature_vectors.shape[0], 256 * P * P))
  ...
  def postprocess_detections(self,
    ...
    for batch_id in range(len(proposals)):
      ...
      per_image_class = class_logits[start_id:start_id +
                                     per_image_proposals.shape[0], :]
      per_image_box = box_regression[start_id:start_id +
                                     per_image_proposals.shape[0], :]
      ...
      per_image_proposals_center = corners_to_centers(per_image_proposals).to(
          self.device)
      ...
      for i in range(3):
        ...
        prediction_box_corner[:, i * 4 + 3] = \
            torch.min(prediction_box_corner[:, i * 4 + 3],
                      img_size[0] * torch.ones(prediction_box_corner[:, i * 4].shape)
                      .to(self.device))
      prediction_box_corner = torch.max(
          prediction_box_corner,
          torch.zeros(prediction_box_corner.shape).to(self.device))

      prediction_box_corner_max = torch.zeros(prediction_box_corner.shape[0],
                                              4).to(self.device)
      prediction_score_max = torch.max(
          per_image_class, dim=1).values.to(self.device)
      ...
      prediction_score_max_sorted_idx = torch.argsort(
          prediction_score_max, descending=True)
      ...
      prediction_label_K = per_image_label[
          prediction_score_max_sorted_idx[:keep_num_preNMS]]
      # print(prediction_score_max_K)

      # Compute NMS
      nms_predict_scores = self.NMS(prediction_score_max_K,
                                    prediction_box_corner_K)
  ...
  def NMS(self, predict_class, prebox):
    ...
    ious = matrix_IOU_corner(
        bbox_mesh_x, bbox_mesh_y, device=self.device).triu(diagonal=1)
  ...
  def regression_loss(self, box_preds: torch.Tensor,
  ...
  def compute_loss(self,
    ...
    if rand_perm_fg is None and rand_perm_bg is None:
      ...
      if torch.sum(labels != 0) > M_no_bg:
        ...
        class_minibatch = class_logits[no_bg_idx[rand_idx]].reshape(
            -1, self.C + 1)
        class_minibatch_gt = labels[no_bg_idx[rand_idx]].reshape(-1, 1)
        reg_minibatch = box_preds[no_bg_idx[rand_idx]].reshape(-1, 4 * self.C)
        reg_minibatch_gt = regression_targets[no_bg_idx[rand_idx]].reshape(
            -1, 4)
      else:
      ...
      if torch.sum(labels == 0) > M_bg:
        ...
        class_minibatch = torch.cat([
            class_minibatch, class_logits[bg_idx[rand_idx]].reshape(
                -1, self.C + 1)
        ],
                                    dim=0)
        class_minibatch_gt = torch.cat(
            [class_minibatch_gt, labels[bg_idx[rand_idx]].reshape(-1, 1)],
            dim=0)
        reg_minibatch = torch.cat([
            reg_minibatch, box_preds[bg_idx[rand_idx]].reshape(-1, 4 * self.C)
        ],
                                  dim=0)
        reg_minibatch_gt = torch.cat([
            reg_minibatch_gt, regression_targets[bg_idx[rand_idx]].reshape(
                -1, 4)
        ],
                                     dim=0)
      else:
        class_minibatch = torch.cat(
            [class_minibatch, class_logits[bg_idx].reshape(-1, self.C + 1)],
            dim=0)
        class_minibatch_gt = torch.cat(
            [class_minibatch_gt, labels[bg_idx].reshape(-1, 1)], dim=0)
        reg_minibatch = torch.cat(
            [reg_minibatch, box_preds[bg_idx].reshape(-1, 4 * self.C)], dim=0)
        reg_minibatch_gt = torch.cat(
            [reg_minibatch_gt, regression_targets[bg_idx].reshape(-1, 4)],
            dim=0)

    else:
      class_minibatch = class_logits[no_bg_idx][rand_perm_fg].reshape(
          -1, self.C + 1)
      ...
      reg_minibatch_gt = regression_targets[no_bg_idx][rand_perm_fg].reshape(
          -1, 4)

      class_minibatch = torch.cat([
          class_minibatch, class_logits[bg_idx][rand_perm_bg].reshape(
              -1, self.C + 1)
      ],
                                  dim=0)
      class_minibatch_gt = torch.cat(
          [class_minibatch_gt, labels[bg_idx][rand_perm_bg].reshape(-1, 1)],
          dim=0)
      reg_minibatch = torch.cat([
          reg_minibatch, box_preds[bg_idx][rand_perm_bg].reshape(
              -1, 4 * self.C)
      ],
                                dim=0)
      reg_minibatch_gt = torch.cat([
          reg_minibatch_gt, regression_targets[bg_idx][rand_perm_bg].reshape(
              -1, 4)
      ],
                                   dim=0)
    ...
    for bz in range(batch_size):
      ...
      for i_pred, class_pred in enumerate(nms_labels[bz]):
        ...
        if class_pred_scalar + 1 in gt_labels[bz]:
          ...
          i_gt_list = (gt_labels[bz] == class_pred +
                       1).nonzero(as_tuple=False).squeeze(0)
...
if __name__ == '__main__':
  ...
  for i in range(7):
    ...
    correctness = labels.cpu().type(
        torch.int8).reshape(-1) == testcase['labels'].type(
            torch.int8).reshape(-1)
    print(labels.type(torch.int8).reshape(-1)[~correctness])
    print(testcase['labels'].type(torch.int8).reshape(-1)[~correctness])
    correctness = torch.abs(testcase['regressor_target'] -
                            regressor_target.cpu()) < 0.01
    # print((~correctness).nonzero())
    print(
        torch.abs(testcase['regressor_target'] -
                  regressor_target.cpu())[~correctness])
  ...
  for num_test in range(4):
    # load test cases
    path = os.path.join(roi_dir,
                        "multiscale_RoIAlign_test" + str(num_test) + ".pt")
    ...
    difference = torch.abs(feature_vectors -
                           output_feature_vectors)[~correctness]
  ...
  for i in range(7):
    ...
    random_permutation_foreground = torch.load(
        path)['random_permutation_foreground']
    random_permutation_background = torch.load(
        path)['random_permutation_background']
  ...
  # proposals: list:len(bz)(per_image_proposals,4) (the proposals are produced from RPN
  #                                                 [x1,y1,x2,y2] format)
  class_logits = torch.tensor([[0.1, 0.2, 0.6, 0.1], [0.4, 0.2, 0.2, 0.2]])
  box_regression = torch.ones(2, 12) * 0.5
  proposals = [
      torch.tensor([[100, 100, 400, 400]]),
      torch.tensor([[200, 200, 600, 600]])
  ]

  boxes, scores, labels = net.postprocess_detections(class_logits,
